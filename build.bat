...
if not defined VSPATH (
    echo [build.bat] Could not find a Visual Studio install with the C++ workload.
    exit /b 1
)
...
rem --- Compile + link ---
cl.exe /EHsc /std:c++latest /nologo /MD /Zi /Fd:build\Chess.pdb /Fo:build\obj\ /Fe:build\Chess.exe ^
  /I "src" /I "src\pieces" ^
